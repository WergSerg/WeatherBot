...
class LocCorr():
    ...
    def parsResp(self, dt):
        try:
            for i in dt:
                try:
                    if not dt['response']['GeoObjectCollection']['featureMember'][0]['GeoObject']['Point']['pos']:
                        self.fee = dt['response']['GeoObjectCollection']['featureMember'][0]['GeoObject']['Point'][
                            'pos']
    ...
    def get_corr(self, city: str):
        resp = YandexApi().getResp(XYandexAPIKey=XYANDEXAPIKEYDIR,
                                   URL=URLDIR.replace('@APIKEY', XYANDEXAPIKEYDIR) + city)
